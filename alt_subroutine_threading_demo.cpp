...
typedef union Instruction * (Engine::*OpCode)( union Instruction * pc );
...
public:
    Instruction *  INCR( Instruction * pc ) {
        if ( DEBUG ) std::cout << "INCR" << std::endl;
        *loc += 1;
        return pc;
    }

    Instruction *  DECR( Instruction * pc ) {
        if ( DEBUG ) std::cout << "DECR" << std::endl;
        *loc -= 1;
        return pc;
    }

    Instruction *  RIGHT( Instruction * pc ) {
        if ( DEBUG ) std::cout << "RIGHT" << std::endl;
        loc += 1;
        return pc;
    }

    Instruction *  LEFT( Instruction * pc ) {
        if ( DEBUG ) std::cout << "LEFT" << std::endl;
        loc -= 1;
        return pc;
    }

    Instruction * PUT( Instruction * pc ) {
        num i = *loc;
    ...
    Instruction * GET( Instruction * pc ) {
    ...
    Instruction * OPEN( Instruction * pc ) {
    ...
    Instruction * CLOSE( Instruction * pc ) {
    ...
    Instruction * HALT( Instruction * pc ) {
